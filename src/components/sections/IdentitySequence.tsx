...
const Step7Counterfactual = ({ visible }: { visible: any }) => (
    <motion.div style={{ opacity: visible }} className="absolute inset-0 z-50 flex items-center justify-center px-5 md:px-10 py-8 pointer-events-none bg-black/95">
        <div className="w-full max-w-6xl rounded-[32px] border border-white/10 bg-zinc-950 p-6 md:p-10 shadow-2xl pointer-events-auto">
            <div className="flex items-center justify-center gap-2 text-[10px] md:text-xs font-bold uppercase tracking-[0.25em] text-zinc-500 mb-5">
                <ShieldCheck className="h-3.5 w-3.5" /> VERITA / 07 · COUNTERFACTUAL VALIDATION
            </div>
            ...
            <p className="mx-auto mt-4 max-w-3xl text-center text-sm md:text-lg text-zinc-400">
                <span className="font-semibold text-zinc-100">This is your killer feature.</span><br />
                ...
                {hypotheses.map((hypothesis, index) => (
                    <motion.div key={hypothesis.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 + index * 0.12, duration: 0.5 }} className={`rounded-3xl border p-5 ${index === 0 ? "border-emerald-400/25 bg-emerald-400/[0.055]" : "border-red-400/20 bg-red-400/[0.035]"}`}>
                        <div className="flex items-start justify-between gap-4">
                            ...
                            <div className={`rounded-full px-3 py-1.5 text-xs font-black tracking-wider ${index === 0 ? "bg-emerald-400/15 text-emerald-300" : "bg-red-400/15 text-red-300"}`}>{hypothesis.status}</div>
                        </div>
            ...
            <div className="mt-4 rounded-3xl border border-white/10 bg-white/[0.035] p-4">
                <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] font-bold text-zinc-500 mb-3"><GitBranch className="h-3.5 w-3.5" /> Financial graph consistency</div>
...
const Step8ImpactModel = ({ visible }: { visible: any }) => (
    <motion.div style={{ opacity: visible }} className="absolute inset-0 z-50 flex items-center justify-center px-5 md:px-10 py-8 pointer-events-none bg-black/95">
        <div className="w-full max-w-6xl rounded-[32px] border border-white/10 bg-zinc-950 p-6 md:p-10 shadow-2xl pointer-events-auto">
            <div className="flex items-center justify-center gap-2 text-[10px] md:text-xs font-bold uppercase tracking-[0.25em] text-zinc-500 mb-5"><ShieldCheck className="h-3.5 w-3.5" /> VERITA / 08 · BREAK PROPAGATION</div>
            <div className="text-center">
                <div className="text-[10px] uppercase tracking-[0.2em] font-bold text-violet-300/70">STEP 8 · BREAK PROPAGATION / FINANCIAL IMPACT MODEL</div>
                <h2 className="mt-3 text-3xl md:text-5xl lg:text-6xl font-black tracking-[-0.04em] leading-tight text-white">What else does this break affect?</h2>
                <p className="mx-auto mt-3 max-w-2xl text-sm md:text-base text-zinc-400">Once you know the cause, determine the downstream financial impact.</p>
            </div>
            ...
            <div className="mt-7 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
                {[
                    ...
                    <motion.div key={label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 + index * 0.08, duration: 0.45 }} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
...
export const IdentitySequence = ({ scrollYProgress, isVisible }: IdentitySequenceProps) => {
    ...
    const step7Opacity = useTransform(localProgress, [0.14, 0.25, 0.36], [0, 1, 0]);
    const step8Opacity = useTransform(localProgress, [0.36, 0.52, 0.68], [0, 1, 0]);
    ...
    return (
        <div className="relative w-screen h-full flex flex-col items-center justify-center overflow-hidden bg-background dark:bg-black">
            ...
            <Step7Counterfactual visible={step7Opacity} />
            <Step8ImpactModel visible={step8Opacity} />
        </div>
    );
};
