...
import { AlertCircle, CheckCircle2, FileImage, FileSpreadsheet, Info, Loader2, Upload, X } from "lucide-react";
...
export function UploadFinanceDashboard({ open, onClose }: Props) {
  ...
  const upload = async () => {
    if (!files.length) return setError("Choose at least one CSV or screenshot first.");
    setBusy(true);
    setError("");
    try {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
      <div className="relative max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-[28px] border border-white/10 bg-[#0a0a0a] text-white shadow-2xl">
        ...
        <div className="border-b border-white/10 px-7 py-7 md:px-10">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-400">Upload one or more source files. Verita creates a private run for this batch, detects each source, validates it, and uses that exact run for the results that follow.</p>
        </div>
        ...
        <div className="grid gap-6 p-7 md:grid-cols-[1.05fr_.95fr] md:p-10">
          <div>
            ...
            <button onClick={() => inputRef.current?.click()} className="group flex min-h-[220px] w-full flex-col items-center justify-center rounded-3xl border border-dashed border-white/20 bg-white/[0.03] p-8 text-center transition hover:border-white/40 hover:bg-white/[0.05]">
              <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.06] group-hover:scale-105 transition"><Upload className="h-6 w-6 text-zinc-200" /></div>
              <div className="text-lg font-bold">Drop files here or browse</div>
              <div className="mt-2 text-xs text-zinc-500">CSV + PNG + JPG + JPEG + WEBP · multiple files allowed</div>
              <div className="mt-5 flex gap-2 text-[10px] font-mono uppercase tracking-wider text-zinc-500"><FileSpreadsheet className="h-3.5 w-3.5" /> CSV <FileImage className="ml-2 h-3.5 w-3.5" /> Screenshot</div>
            </button>
            <input ref={inputRef} type="file" multiple accept=".csv,.png,.jpg,.jpeg,.webp,image/*,text/csv" className="hidden" onChange={(e) => chooseFiles(e.target.files)} />
            ...
            <div className="mt-5 flex items-start gap-3 rounded-2xl border border-sky-400/20 bg-sky-400/[0.05] p-4 text-xs leading-5 text-sky-100/80"><Info className="mt-0.5 h-4 w-4 shrink-0 text-sky-300" /><span><b className="text-sky-200">Screenshot tip:</b> upload a clear screenshot where the column headers and rows are visible. Gemini Vision will extract the table into the same canonical finance schema before reconciliation.</span></div>
          ...
          <div>
            ...
            <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.025] p-5"><div className="text-xs font-bold text-zinc-300">You don't need to upload all four.</div><p className="mt-2 text-xs leading-5 text-zinc-500">Upload the records you have. Missing sources are shown as missing and the controller will not fabricate them. More source types = a richer reconciliation chain.</p></div>
