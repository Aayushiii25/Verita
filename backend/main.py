...
def ai_insights():
    """Run the trained record-linking model and return explainable AI signals."""
    bank, stl, inv, led = load_data_from_csv()
    matches = engine.reconcile(bank, stl, inv, led)
...
def ai_exception(bank_id: str):
    """Return a decision trace for one bank transaction."""
    bank, stl, inv, led = load_data_from_csv()
    matches = engine.reconcile(bank, stl, inv, led)
...
def ai_impact():
    """Estimate downstream financial impact of unresolved reconciliation breaks."""
    bank, stl, inv, led = load_data_from_csv()
    matches = engine.reconcile(bank, stl, inv, led)
    return build_impact(matches, bank, stl)


@app.get("/api/ai/benchmark")
def ai_benchmark():
    """Compare model/controller classifications with the synthetic ground truth."""
    bank, stl, inv, led = load_data_from_csv()
    matches = engine.reconcile(bank, stl, inv, led)
